				...
				if workspace.Portal.Mid:GetChildren()[6].Enabled == false then
					...
					if not game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("WaveUI") then
						...
						for i, v in pairs(workspace.DunMob:GetChildren()) do
							if v and vFindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
							    repeat task.wait()
							        Attack()
							        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * MethodFarm
							    until not v and not v:FindFirstChild("Humanoid") and not v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health <= 0
							else
							    repeat task.wait() until v and vFindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0
							end
						end
					end
				end
