				...
				if workspace.Portal.Mid:GetChildren()[6].Enabled == false then
					...
					if not game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("WaveUI") then
						...
						for i, v in pairs(workspace.DunMob:GetChildren()) do
							if not game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("WaveUI").Wave.Text:lower():find("WAVE") or (v.Name == "Mahito" and v.Humanoid.Health <= 0) then
								repeat task.wait()
									game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(18491.767578125, 48.0782470703125, -907.4343872070312)
								until game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("WaveUI").Wave.Text:lower():find("WAVE") or (v and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart"))
							else
								v.Humanoid.WalkSpeed = 0
								v.Humanoid.JumpPower = 0
								repeat
