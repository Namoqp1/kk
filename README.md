				...
				if workspace.Portal.Mid:GetChildren()[6].Enabled == false then
					...
					if not game:GetService("Players").LocalPlayer.PlayerGui:FindFirstChild("WaveUI") then
						...
						for i, v in pairs(workspace.DunMob:GetChildren()) do
							if v and v:IsA("Model") and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") then
								repeat task.wait()
									game.Players.LocalPlayer.Character:MoveTo(v.HumanoidRootPart.CFrame.Position * MethodFarm)
								until not v 
							end
						end
					end
